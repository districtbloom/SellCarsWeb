...
export class CarInstance {
  ...
  constructor(scene: Scene, readonly id: number, world?: World) {
    ...
    this.physics = new VehiclePhysics(this.rig.spec, { ...profile.tuning, ...this.rig.tuning }, world);
  ...
  sync() {
    ...
    this.car.quaternion.set(body.quaternion.x, body.quaternion.y, body.quaternion.z, body.quaternion.w);
    const offset = this.rig.centerOfMass.clone().multiply(this.rig.scale).applyQuaternion(this.car.quaternion);
    this.car.position.set(body.position.x, body.position.y, body.position.z).multiplyScalar(1 / METERS_PER_UNIT).sub(offset);
    this.wheels.forEach((pivot, index) => {
      const { position, quaternion } = this.physics.wheelTransform(index);
      pivot.position.set(position.x, position.y, position.z).multiplyScalar(1 / METERS_PER_UNIT);
      pivot.quaternion.set(quaternion.x, quaternion.y, quaternion.z, quaternion.w);
    });
  }
